...
MissAV の詳細ページで HLS master playlist を取得し、選択した解像度をローカルのデスクトップクライアント [M3U8-Downloader](https://github.com/lengziyu/m3u8-downloader) に送る Chrome Manifest V3 拡張です。
...
- `src/adapters/missav.js`: MissAV 用サイトアダプター
...
6. MissAV の詳細ページを開き、プレイヤーを一度読み込ませる
...
### 方法 2: Chrome Web Store

1. 拡張をパッケージ化して Chrome Web Store にアップロード
2. ストア情報、権限説明、審査を完了
3. ユーザーはストアから直接インストール

一般ユーザーにはこちらが適していますが、ローカルクライアント [M3U8-Downloader](https://github.com/lengziyu/m3u8-downloader) は別途必要です。

## 動作フロー

1. content script が MissAV の詳細ページを判定し、ページ情報を取得
